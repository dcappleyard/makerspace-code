# Web-interface panorama photo frame

Project directory: `esp32-projects/panorama_photo_frame_web/`. Built as a copy of
`../panorama_photo_frame_w_counter/`, which remains in the repo and still works.

> **This section supersedes the firmware architecture described below.** The
> original design document is kept underneath because its *reasoning* is still
> the best record of why the photo geometry, the `.bin` contract, the sidecar
> metadata, and the deferred-counter-redraw behavior are what they are — all of
> which carry over unchanged. What no longer applies is everything downstream of
> the battery assumption.

## Why this exists

The original frame was designed around battery operation, and that single
constraint shaped its whole architecture: all work happened in `setup()`, which
always ended in `esp_deep_sleep_start()`; `loop()` was unreachable; buttons were
deep-sleep `ext1` wake sources; the SD card was mounted only for sub-second
windows; and there was no WiFi at all, which is why the clock had to be pasted
in over serial on every cold boot.

Battery life didn't come out as hoped and the frame now lives permanently
plugged in. That retires the constraint — and with it the awkward parts it
forced. This version spends the freed power budget on being reachable.

## What carries over untouched

The entire display path. `.bin` format (1,314,144 bytes, nibble-packed, gray
0-15), `/pictures/` filename ordering, the `.xml` sidecar field names including
the per-corner `text_color_*` values, `drawTextOverlay()` and its hand-drawn
separators, the 6:17 strip geometry and its hand-sync with `prepare_image.py`,
and the deferred-counter-redraw rule. `prepare_image.py` needs no changes.

## Decisions

**Always awake; deep sleep removed entirely.** A synchronous `WebServer` needs a
tight `handleClient()` loop, which is fundamentally incompatible with
`esp_deep_sleep_start()` (that never returns — a wake is a reboot into
`setup()`). A hybrid "wake into a timed web window" was considered and rejected:
it would preserve battery capability, but the web UI would be unreachable
almost all the time, which defeats the point of uploading from your desk.

**The refresh deadline is `millis()`, not an epoch.** This is the one decision
that would be silently wrong the other way. The ESP32 boots at epoch 0 and NTP
steps the clock by decades within seconds of connecting. An epoch deadline
computed before that sync is instantly overdue afterwards — a refresh storm on
every boot — and a backward correction would push it years out. `millis()` is
monotonic and immune to `settimeofday()`; the unsigned-subtraction idiom handles
the 49.7-day rollover, and `refresh_hours` is clamped to 168 to stay well clear
of it. Wall-clock time is used only for *display* and the counter-track
timestamp, never for scheduling.

**The current photo is persisted by name, not index.** `listPictures()` sorts by
filename, so uploading a photo that sorts early shifts every index. The old
`imgIndex` would have silently resumed on the wrong photo after any upload.

**No persisted clock-validity flag.** The old `rtcValid` NVS bit made sense when
deep sleep preserved the RTC domain across resets. Now a reset is always a
genuine power loss, so a stored flag would report "valid" for a clock that had
just fallen back to 1970. Validity is derived live by sanity-checking the epoch
instead. NVS is down to a single key.

**Everything runs in one task, and that is the load-bearing assumption.**
`server.handleClient()` and the panel refresh both execute from `loop()`, so
they can never interleave. There is no concurrency here — only latency to
manage and ordering to get right. That's why no part of this needs a lock; why
a web handler can safely mutate state a refresh will later read; why an upload
can't collide with a refresh (it just waits a couple of seconds in the accept
backlog); and why the `SD.remove()`/`SD.rename()` gap in the upload commit is
safe rather than a race. Anyone adding a task or an ISR later invalidates all
four of those and needs to revisit them.

**Web handlers never block on the panel.** A handler sets a priority-ordered
pending flag and returns a 303 immediately; `loop()` does the multi-second
refresh. Requests arriving close together coalesce into a single refresh with
the most significant action winning. The 3-second gray settle became a timer
gate rather than a `delay()`, which costs nothing (only `performRefresh()` ever
touches the panel) and cuts the unresponsive window from ~5 s to ~1.5-2 s.

**Uploads are all-or-nothing.** Both files stream to temps at the card root and
are renamed into `/pictures/` only after every check passes. Any error at all
commits nothing. A half-updated pair would show a new photo under the old
caption — worse than no update. The exact 1,314,144-byte length check is the
critical one, because the display path hard-fails a short read and would blank
that slot.

**A scheduled refresh with no card fails silently.** The old firmware drew an
error screen unconditionally, which was fine when it was about to sleep anyway.
Now, wiping a perfectly good photo to announce a missing card is worse than
showing a stale one. Error screens are reserved for boot-with-nothing-on-screen
and explicitly requested refreshes.

**No authentication, with a one-line seam to add it.** Chosen deliberately,
matching `../wifi_counter_eink`. The exposure is materially larger than that
project's, though — uploads write to the filesystem — so it's documented plainly
in the README rather than buried, and `requireAuth()` guards every route already
so two `#define`s in `secrets.h` turn it on.

**`app0` uses subtype `ota_0`.** Not cosmetic: PlatformIO's
`_update_max_upload_size()` only lowers the reported limit for an `ota_0`
partition, so with `factory` (as the sibling uses) an over-large firmware
flashes cleanly and then fails to boot with no warning. This build links the
whole WiFi stack, so a real size check matters. `app1` reserves OTA space for
later; OTA itself is not implemented.

## Rejected or deferred

- **Chunking the SD image read** across `loop()` passes to shave the remaining
  ~1.5 s stall. Raising the SD clock from 4 to 20 MHz was a 10× cheaper win for
  the same problem; revisit only if the stall actually bothers you.
- **Overriding `HTTP_UPLOAD_BUFLEN`** to speed uploads. It changes
  `sizeof(HTTPUpload)`, and a mismatch between translation units is a silent ODR
  violation and heap corruption. Not worth it before measuring that 1436-byte
  writes are the bottleneck.
- **`tconWaitForDisplayReady()`** instead of the 3-second settle. It's public,
  but its body is an untimed `while (tconReadReg(LUTAFSR));` and `update()` has
  already told the panel to sleep by the time you could call it. The failure mode
  is a permanent hang, not a glitch.
- **A boot splash** showing the IP. It costs a full extra gray refresh on every
  power-up for information the panel's top-right corner carries seconds later.
- **Photo deletion over the web.** Out of scope by choice; manage the set by
  pulling the card.

---

# Inherited design: the battery/deep-sleep frame
